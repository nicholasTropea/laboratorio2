#! /usr/bin/env python3
# IMPORTANT, MAKE SURE BOTTOM RIGHT SAYS LF INSTEAD OF CRLF

import sys # Permette di passare parametri da riga di comando

# IMPORTANTE PER LA SHELL PER SAPERE COME DEVE ESEGUIRE IL FILE
# chmod +x filename
...
# il contenuto della linea di comando si trova in sys.argv, grazie ad import sys

if len(sys.argv) == 2:
  main(int(sys.argv[1]))
else:
  print(f"Uso: {sys.argv[0]} interoPositivo")  


# Dictionary keys MUST be immutable objects
# Instead of using a list (mutable) we can use a tuple (immutable)

# dict.keys() returns a list of the dictionary keys 
