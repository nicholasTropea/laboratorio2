...
def createSubDirs(main, list, dest):
    """
    Creates dest/* subdirectories and links and adds the links to the Miofile objects
    """
    ...
    for file in list:
        ...
        subDir = name[0].lower() # Get subdirectory name
        ...
        if name not in created:
            ...
            linkName = f"{name}.{created[name]}" # Update link name
            created[name] += 1 # Increment index
        
        os.link(file.path, linkName) # Create the link
        file.linkPath = os.path.abspath(os.path.join(main, dest, subDir, linkName)) # Save link path
...
# Start the program
if len(sys.argv) != 3:
    print(f"Error: Incorrect usage.\nCorrect usage: {sys.argv[0]} <src> <dest>")
else:
