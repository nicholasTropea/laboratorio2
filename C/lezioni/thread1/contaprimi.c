#include "xerrori.h"
#include <pthread.h> // Need to compile with -pthread
...
int main(int argc, char *argv[]) {
  ...
  // Creazione thread ausiliari (array statici dato che #thread limitato)
  pthread_t t[p];   // array di p indentificatori di thread 
  dati d[p];        // array di p struct che passerò ai p thread

  int somma = 0; // Accumulatore #primi trovati

  for(int i = 0; i < p; i++) {
    int n = m / p;  // #Numeri per thread
